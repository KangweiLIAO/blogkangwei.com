---
title: Recursive vs Iterative Solutions in Linked List Problems
date: '2024-07-15'
tags: ['leetcode', 'recursion', 'iteration', 'linked lists', 'space complexity']
draft: false
summary: A discussion on choosing between recursion and iteration with linked list problems.
---

In this post, I want to dive deep into the decision-making process behind choosing between recursion and iteration, using a linked list problem as an example. The problem here is to find the intersection node of two singly linked lists. Let’s first review two solutions—one using recursion and the other using iteration.

## Problem Definition

We are given two singly-linked lists, and we need to determine if they intersect at some node. If they do, we return that node; otherwise, we return `None`.
...
class Solution1:
    def getIntersectionNode(self, headA: ListNode, headB: ListNode) -> Optional[ListNode]:
        ...
        def helper(headA: ListNode, headB: ListNode, lenA: int, lenB: int) -> Optional[ListNode]:
            ...
            # both lists are aligned in length
